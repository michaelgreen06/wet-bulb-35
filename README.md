...
`workers/hono-page-renderer.mjs` is a static HTML/UX renderer for the isolated Workers staging candidate. The non-deployed weather-edge candidate is configured separately in `wrangler.weather-staging.toml`: it binds one global `WeatherGate` Durable Object and intentionally has no `OPENWEATHER_API_KEY`, so `/api/weather` fails closed in local/staging verification. Browser responses remain private/no-store while internal Cache API/DO envelopes use proposed 300-second fresh, 600-second stale, and 5-second timeout defaults. The global daily provider-attempt ceiling remains disabled until Michael explicitly approves its value. `npm run dry-run:weather-edge` packages only this isolated configuration and does not deploy it.
