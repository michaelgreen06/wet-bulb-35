...
`workers/hono-page-renderer.mjs` is a static HTML/UX renderer for the isolated Workers staging candidate. Successful recognized `GET` HTML responses use a separate, schema-validated Cache API envelope: 24 hours fresh plus 7 days stale with an explicit 8-day internal storage TTL. Browser HTML deliberately keeps the captured parity header `Cache-Control: public, max-age=0, must-revalidate`; it is reconstructed from, and never stored as, the internal envelope. `HTML_CACHE_VERSION` is the deployment/content invalidation namespace. The non-deployed weather-edge candidate is configured separately in `wrangler.weather-staging.toml`: it binds one global `WeatherGate` Durable Object and intentionally has no `OPENWEATHER_API_KEY`, so `/api/weather` fails closed in local/staging verification. Browser weather responses remain private/no-store while internal weather Cache API/DO envelopes use the approved initial 300-second fresh, 600-second stale, and 5-second timeout values. The approved initial global ceiling is 100 reserved provider attempts per UTC day. `npm run dry-run:weather-edge` packages only this isolated configuration and does not deploy it.
