...
class LocationPackagingProbeTests(unittest.TestCase):
    ...
    def test_compact_candidate_requires_production_route_identity(self):
        colliding_rows = [
        ...
        compact_rows = [[row[field] for field in probe.COMPACT_FIELDS] for row in colliding_rows]

        self.assertEqual([row[0:2] for row in compact_rows], [["Metsamor", "Armavir"], ["Metsamor", "Armavir"]])
        self.assertNotIn("outputCitySlug", probe.COMPACT_FIELDS)
        self.assertEqual(
            probe.ROUTE_IDENTITY_CONTRACT["required_generated_input"],
            "outputCitySlug from production JS prepareCities, or a full route index generated by that code",
        )
