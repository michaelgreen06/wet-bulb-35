...
Its `encrypted-snapshot` operation processes one deterministic 100-URL shard selected from `0` through `3`, packages the private SQLite and CSV files, and encrypts the package with the repository's public recipient certificate before upload. Four shards cover the 400 URLs exactly once and can be decrypted and combined locally. Sharding keeps each run resumable and comfortably below the job timeout. Concurrency, duplicate-shard, conservative daily-budget, and rerun guards prevent separate ephemeral runners from bypassing the URL Inspection limits. The matching private decryption key lives outside Git on the Hermes host. The public-repository artifact is ciphertext only, expires after seven days, and cannot be decrypted by GitHub or repository readers. After downloading the artifact, decrypt it locally with:
