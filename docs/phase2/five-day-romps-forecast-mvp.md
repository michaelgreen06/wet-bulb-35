...
The free Open-Meteo endpoint is restricted to non-commercial use under its current terms. Advertising, subscriptions, or other commercial operation requires the appropriate customer endpoint and licence before launch. `OPEN_METEO_API_MODE` is explicitly `public-noncommercial` on staging and `disabled` in both production configurations, so an accidental production deployment cannot call Open-Meteo. Production enablement requires a reviewed change to either `public-noncommercial` after confirming eligibility or `customer-commercial` with an `OPEN_METEO_API_KEY` secret. The key must be configured with Wrangler's secret store and never committed.
