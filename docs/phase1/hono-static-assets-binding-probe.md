...
- Deterministic three-row fixtures cover country, state, one unique city, both generated Metsamor collision paths, and a missing city. Each recognized slashful and slashless route returns `200`; each HTML document has the slashful canonical URL.
...
- `tests/hono-binding-probe.test.mjs` also starts `wrangler dev --local` (Wrangler/Miniflare) and makes real HTTP requests against the static-assets binding. It is not solely a direct function test.

## Staging configuration

`wrangler.staging.toml` declares only the generated `ASSETS` binding, a probe-only name, main module, and compatibility date. It contains no `route`, `routes`, `custom_domain`, secret, provider URL, or production cache binding. It is intended only for the non-deploying commands below.
...
| `wrangler deploy --dry-run` | Wrangler 4.129.1 read **227 files** from the assets directory; Worker bundle **64.57 KiB**, gzip **16.06 KiB**; binding shown as `env.ASSETS`; exited with `--dry-run: exiting now` |
| Local startup | **1,592.715 ms** from Wrangler spawn to first local `404` readiness response, using the small fixture assets |
| Local integration | 3/3 tests passed; both forms of 5 recognized fixture routes returned `200`; missing route returned `404` |

## Limitations / gate outcome

- The 1,592.715 ms measurement includes local Wrangler process startup, Miniflare initialization, and the polling granularity. It is **not** Cloudflare cold-start evidence and exceeds the Phase 1 target-serving-model's 1 s startup gate if treated as a comparable number. A real staging deployment and an approved staging-start measurement remain a blocker before workers.dev deployment.
