...
5. API unit/integration tests cover GET success; missing/non-finite coordinates (`400`); `HEAD`, `OPTIONS`, `POST`, and another verb (each `405`, exact `Allow: GET`); `/api/weather/` (`404`, no redirect); bot (`204`, zero provider calls); canonical equivalent coordinates sharing one key with `-0` normalized to `0`; exact parsed numbers forwarded upstream; fresh hit (zero provider calls); isolate/colo-local best-effort same-key concurrent-miss collapse; stale hit with successful refresh; stale hit with failed refresh; miss failure (`500`, not cached); timeout; and the internal envelope timestamps. They must not assert global collapse or global freshness.
