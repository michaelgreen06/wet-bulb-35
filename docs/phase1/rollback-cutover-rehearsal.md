...
- The retained inventory observation is **0** production-zone Worker routes. It is evidence, not permission to alter the zone and not a current-state assertion.
...
## Disposable workers.dev/version rehearsal (not executed here)
...
5. Evidence will contain only version IDs, command exit status, UTC timestamps, and the fixed-schema output from `npm run tail:weather-staging-safe`. Raw Wrangler output, request URLs, bodies, headers, IPs, and secrets are prohibited.
