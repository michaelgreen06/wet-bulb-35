...
function renderForecastWidget(routePath, cityName) {
  if (!popular40EnrichmentByPath.has(routePath)) return "";
  return `<section class="rounded-lg border border-gray-200 bg-white p-6" aria-labelledby="wet-bulb-forecast-heading" data-forecast-widget data-forecast-path="${escapeHtml(routePath)}">
    <div class="flex flex-wrap items-baseline justify-between gap-2">
      <h2 id="wet-bulb-forecast-heading" class="text-2xl font-bold text-gray-800">Five-day wet bulb forecast for ${escapeHtml(cityName)}</h2>
    ...
    <div class="mt-5 border-t border-gray-200 pt-4 text-sm text-gray-600">
      <p>Wet bulb values are calculated from hourly temperature, dew point, and surface pressure using the Romps thermodynamic liquid-water method.</p>
...
export function pageHtml(city, options = {}) {
  ...
  const hasForecast = options.forecastEnabled === true && popular40EnrichmentByPath.has(routePath);
  const description = hasForecast
    ? `Get the current wet bulb temperature and five-day wet bulb forecast for ${fullName}.`
...
${placesRuntime}
  ...
  function renderForecast(widget, payload) {
    ...
    const updated = widget.querySelector("[data-forecast-updated]");
