...
function renderClimateContext(city, routePath) {
  ...
  const peaks = enrichment.nasaPower.peakMonths.map((month) => monthNames[month - 1]).join(", ");
  ...
  return `<section aria-labelledby="city-climate-context-heading" class="rounded-lg border border-gray-200 bg-white p-6">
    ...
    <p class="text-gray-700">The 1991–2020 Köppen-Geiger map classifies the area around ${escapeHtml(city.name)} as ${escapeHtml(enrichment.koppenGeiger.label.toLowerCase())} (${escapeHtml(enrichment.koppenGeiger.code)}).</p>
    <dl class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 text-sm text-gray-700">
      <div><dt class="font-semibold">IANA timezone</dt><dd>${escapeHtml(enrichment.geonames.timezone)}</dd></div>
      <div><dt class="font-semibold">Approximate elevation</dt><dd>${escapeHtml(elevation)}</dd></div>
    </dl>
    <p class="mt-5 text-gray-700">NASA POWER's modeled 1991–2020 climatology estimates the highest monthly mean wet bulb temperature in ${escapeHtml(peaks)} at ${peakValue} °C.</p>
    <div class="mt-4 overflow-x-auto"><table class="w-full border-collapse text-sm text-gray-700"><caption class="mb-2 text-left font-semibold text-gray-800">Modeled monthly mean wet bulb temperature, 1991–2020</caption><thead><tr class="border-b border-gray-200"><th scope="col" class="px-3 py-2 text-left">Month</th><th scope="col" class="px-3 py-2 text-right">Wet bulb</th></tr></thead><tbody>${monthlyRows}</tbody></table></div>
    <p class="mt-3 text-sm text-gray-600">NASA POWER values use a coarse MERRA-2 grid and local solar time. They are modeled monthly means, not station observations or records, and may not reflect neighborhood conditions. Köppen-Geiger classification is sampled at the city-center grid cell and checked against the surrounding 3×3 cells.</p>
    <p class="mt-2 text-sm text-gray-600">Sources: <a class="text-blue-600 hover:underline" href="https://www.geonames.org/" target="_blank" rel="noopener noreferrer">GeoNames</a>; <a class="text-blue-600 hover:underline" href="https://doi.org/10.6084/m9.figshare.21789074.v3" target="_blank" rel="noopener noreferrer">Beck et al. Köppen-Geiger</a>; <a class="text-blue-600 hover:underline" href="https://power.larc.nasa.gov/" target="_blank" rel="noopener noreferrer">NASA POWER</a>.</p>
  </section>`;
}
...
export function pageHtml(city, options = {}) {
  ...
  const description = `Get the current wet bulb temperature for ${fullName}.`;
  ...
  const cityContent = `
    ...
    ${renderWeatherWidget({
      ...
      lon: Number(city.longitude),
      mode: "city",
    })}${renderClimateContext(city, routePath)}
    ${methodology}
  `;
