...
export function createSiteData(sourceCities, tier1Manifest = null) {
  ...
  const annotatedCities = cities.map((city) => ({ ...city, tier1: tier1.get(routePathForCity(city)) ?? null }));
  ...
  for (const city of annotatedCities) {
    ...
    if (city.tier1) countriesMap.get(countryKey).tier1Count += 1;
    countriesMap.get(countryKey).states.push(city.resolvedAdmin1Code);
    statesMap.get(stateKey).cities.push(city);
    if (city.tier1) statesMap.get(stateKey).tier1Count += 1;
  }
      ...
      states: sortByName(
        dedupeBy(
          country.states.map((stateName) => ({
            ...
            tier1Count: statesMap.get(`${country.slug}/${toSlug(stateName)}`).tier1Count,
          })),
          (state) => state.slug,
        ),
      ),
  ...
  const states = [...statesMap.values()]
    .map((state) => ({
      ...
      cities: [...state.cities].sort((a, b) => {
        const tierCompare = Number(Boolean(b.tier1)) - Number(Boolean(a.tier1));
...
export function renderCountryPage(country, options = {}) {
  ...
  const stateCards = [...country.states].sort((a, b) => (b.tier1Count || 0) - (a.tier1Count || 0) || a.name.localeCompare(b.name)).map(
